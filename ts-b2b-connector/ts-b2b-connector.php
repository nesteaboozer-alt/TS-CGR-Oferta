 ...
 * Description: Obsługa Partnerów B2B: Absolutna separacja (XOR), blokada linków i wbudowana diagnostyka.
 * Version: 3.2.0
 * Author: TechSolver
 */
...
class TS_B2B_Connector {
    ...
    public static function init() {
        ...
        // 2. MUR PRODUKTOWY (FILTROWANIE GLOBALNE)
        add_action( 'pre_get_posts', array( __CLASS__, 'filter_products_globally' ), 9999 );
        
        // 3. MUR PRODUKTOWY (BLOKADA LINKÓW)
        add_action( 'template_redirect', array( __CLASS__, 'block_direct_access' ) );

        // 4. SYNCHRONIZACJA WIDOCZNOŚCI (DLA FRONT-LISTY I WC)
        add_filter( 'woocommerce_product_is_visible', array( __CLASS__, 'check_final_visibility' ), 9999, 2 );

        // 5. PROFIL I DANE ZABLOKOWANE
        ...
        // 6. BRAMKA PŁATNOŚCI I STYLE
        add_filter( 'woocommerce_payment_gateways', array( __CLASS__, 'register_gateway' ) );
        add_action( 'wp_head', array( __CLASS__, 'b2b_checkout_styles' ), 30 );

        // 7. DIAGNOSTYKA (Wbudowana)
        add_action( 'wp', array( __CLASS__, 'run_diagnostic_tool' ) );
    ...
    /**
     * Helper: Czy to Partner B2B (Admin nigdy nie jest traktowany jako B2B w filtrach)
     */
    ...
    public static function filter_products_globally( $query ) {
        ...
        if ( self::is_strictly_b2b() ) {
            // PARTNER B2B: Widzi TYLKO B2B
            $meta_query[] = array( 'key' => self::META_B2B_ONLY, 'value' => 'yes', 'compare' => '=' );
        } else {
            // GOŚĆ: Nie widzi B2B
            $meta_query[] = array(
                'relation' => 'OR',
    ...
    public static function b2b_checkout_styles() {
        if ( is_checkout() && self::is_strictly_b2b() ) {
            echo '<style>.apple-invoice-toggle, #billing_want_invoice_field { display: none !important; } #billing_company_field, #billing_nip_field { display: block !important; opacity: 1; } .woocommerce-checkout { background: #fff !important; } input[readonly] { background: #f9f9f9 !important; border: 1px dashed #ccc !important; }</style>';
        }
    }
    ...
    public static function add_b2b_endpoint() { add_rewrite_endpoint( 'b2b-services', EP_PAGES ); }
    ...
    /**
     * NARZĘDZIE DIAGNOSTYCZNE: Wywołaj ?ts_debug=b2b jako Admin
     */
    public static function run_diagnostic_tool() {
        if ( ! isset( $_GET['ts_debug'] ) || $_GET['ts_debug'] !== 'b2b' ) return;
        if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Dostęp tylko dla Administratora.' );

        if ( isset( $_GET['clear_cache'] ) ) { wc_delete_product_transients(); echo '<b>Wyczyszczono cache WC!</b><br>'; }

        $user = wp_get_current_user();
        echo '<h1>Diagnostyka B2B</h1>';
        echo 'Rola B2B w systemie: ' . ( get_role( self::B2B_ROLE ) ? 'OK' : 'BRAK' ) . '<br>';
        echo 'Twój login: ' . $user->user_login . '<br>';
        echo 'Twoje role: ' . implode( ', ', $user->roles ) . '<br>';
        echo 'Czy system widzi Cię jako B2B?: ' . ( self::is_strictly_b2b() ? 'TAK (Filtry aktywne)' : 'NIE (Widzisz wszystko jako Admin)' ) . '<br><br>';
        echo '<a href="' . add_query_arg( 'clear_cache', '1' ) . '">WYCZYŚĆ CACHE WIDOCZNOŚCI (WC TRANSIENTS)</a><hr>';

        echo '<h3>Próbka Produktów:</h3><table border="1" cellpadding="5" style="border-collapse:collapse;"><tr><th>ID</th><th>Nazwa</th><th>Meta B2B</th><th>Widoczny?</th></tr>';
        $test_products = wc_get_products( array( 'limit' => 20 ) );
        foreach ( $test_products as $p ) {
            $meta = get_post_meta( $p->get_id(), self::META_B2B_ONLY, true );
            echo "<tr><td>{$p->get_id()}</td><td>{$p->get_name()}</td><td>$meta</td><td>" . ( $p->is_visible() ? 'TAK' : 'NIE' ) . "</td></tr>";
        }
        echo '</table>';
        exit;
    }
